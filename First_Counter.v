...
module First_Counter(
    input Clk, Reset,
    output Q,      //'Q' will be 1-bit since it will increment the second counter
    output [3:0] Display_One
    );
    
  // Make sure your Counter is driven by  slow clock generated by clock divider
reg[3:0] Present_State, Next_State;
//wire First_Counter_Output;         //Connects to second counter, port 'Clk'
...
parameter S10 = 4'b0000; //Dummy state for tens change
...
always@(*)              //Takes all inputs and watches if they change
    ...
    S9: Next_State = S10;    // Transitions from 9 -> 0
    S10: Next_State = S1;   // Transitions form 0 -> 1
    
    default: Next_State = S0;   //Stops errors from occuring 
endcase

assign Q =(Present_State == S10)? 1'b1:1'b0;   // Syntax for defining bits:
                                              // (Number of bits)'b(Define Value)
                                              // (b) stands for binary
assign Display_One = Present_State[3:0];    //Displays number on 7-Segment    
